...
public class GenerateElectricityBill {
    ...
    void generateInvoice(MouseEvent event) {
        ...
        try {
            File pdfBillName = new File("target/", session.getSupplierName()+"-"+month.toLowerCase()+"-bill"+".pdf");
            ...
            document.add(new Paragraph("Customer FullName:     "+ session.getSupplierName()));
    ...
    @FXML
    public void sentByEmail(MouseEvent event) {
        String[] recepients = {"frehman1993@outlook.com"};
        String[] bccRecepients = {};
        String subject = "Hello There!";
        String message = "How are you?";
        new Utils().sendMail(recepients, bccRecepients, subject, message);
        //new Utils().send();
    }
    ...
    private Bill calculateBill(String newValue, String field) {
        ...
        return new Bill("Electricity", customer.getId(), customer.getFullName(), firstDate+" - "+lastDate, opening, closing, 0.20, read, totalBill);
    }
